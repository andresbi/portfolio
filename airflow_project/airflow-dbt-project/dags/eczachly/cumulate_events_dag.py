...
def cumulative_web_events_dag():
    ...
    production_table = 'bootcamp.user_web_events_cumulated'
        ...
        op_kwargs={
            'query': f"""
                ...
                SELECT user_id, 
                        academy_id, 
                        event_count_array,
                         COALESCE(ELEMENT_AT(event_count_array, 1),0) +
                         COALESCE(ELEMENT_AT(event_count_array, 2), 0)
                        +  COALESCE(ELEMENT_AT(event_count_array, 3),0)
                        +  COALESCE(ELEMENT_AT(event_count_array, 4),0)
                        +  COALESCE(ELEMENT_AT(event_count_array, 5),0)
                        +  COALESCE(ELEMENT_AT(event_count_array, 6),0)
                        +  COALESCE(ELEMENT_AT(event_count_array, 7),0) as  event_count_last_7d,
